...
import { Card, Select, Icon, Slider, Button, Input, List, Avatar, Skeleton, Modal } from "antd";
...
export class SearchComponent extends Component {
  ...
  componentWillMount() {
    ...
    if (_recivedData !== undefined) {
      ...
      if (window.location.href.indexOf("search") > -1) {
        ...
        Api.SearchSim(_recivedData).then(res => {
        ...
        {!this.state.loading ? (
          <React.Fragment>
            {this.state.data.length >= 1 ? <div>
              ...
              >
                ...
                >
                  <div className="payModal">
                    <h3>
                      <div>  شما قصد خرید شماره <span>{this.state.selectedProduct.Number}</span> را دارید <Icon type="info-circle" style={{ fontSize: 24, verticalAlign: "middle" }} /></div>
                    </h3>
                    <h4>
                      <Icon type="barcode" style={{ fontSize: 24, verticalAlign: "middle" }} />   مبلغ : {this.state.selectedProduct.Price} ریال
                        </h4>
                    <p>
              ...
              <div>
                {window.location.href.indexOf("search") > -1 ? <p style={{ textAlign: "center" }}>
                  نتیجه ای یافت نشد
            </p> : ""}
              </div>}
          </React.Fragment>
  ...
  onMoreClick = () => {
    ...
    Api.SearchSim(searchItems).then(res => {
  ...
  onPayment = () => {
    ...
    if (_token) {
        Api.Pay(obj).then(res => { console.log(res) }).catch(err => console.log(err))
    }
    else {
